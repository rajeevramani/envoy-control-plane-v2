...
async fn test_cluster_to_proto_conversion() {
    ...
    let decoded =
        envoy_types::pb::envoy::config::cluster::v3::Cluster::decode(&proto_cluster.value[..])
            .unwrap();
...
async fn test_route_to_proto_conversion() {
    ...
    assert_eq!(
        proto_route.type_url,
        "type.googleapis.com/envoy.config.route.v3.RouteConfiguration"
    );
    assert!(!proto_route.value.is_empty());

    // Verify we can decode the protobuf
    let decoded = envoy_types::pb::envoy::config::route::v3::RouteConfiguration::decode(
        &proto_route.value[..],
    )
    .unwrap();
...
async fn test_cluster_with_single_endpoint() {
    ...
    let decoded =
        envoy_types::pb::envoy::config::cluster::v3::Cluster::decode(&proto_clusters[0].value[..])
            .unwrap();
...
async fn test_multiple_clusters_conversion() {
    ...
    let decoded1 =
        envoy_types::pb::envoy::config::cluster::v3::Cluster::decode(&proto_clusters[0].value[..])
            .unwrap();
    let decoded2 =
        envoy_types::pb::envoy::config::cluster::v3::Cluster::decode(&proto_clusters[1].value[..])
            .unwrap();
