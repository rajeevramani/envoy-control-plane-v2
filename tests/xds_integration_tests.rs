...
async fn test_get_resources_by_type_clusters() {
    ...
    assert_eq!(resources[0].type_url, "type.googleapis.com/envoy.config.cluster.v3.Cluster");
}
...
async fn test_get_resources_by_type_routes() {
    ...
    assert_eq!(resources.len(), 1);
    assert_eq!(resources[0].type_url, "type.googleapis.com/envoy.config.route.v3.RouteConfiguration");
}
