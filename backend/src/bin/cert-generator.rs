...
fn main() -> Result<()> {
    ...
    let cert_dir = "certs";
    let cert_path = format!("{cert_dir}/server.crt");
    let key_path = format!("{cert_dir}/server.key");

    // Generate private key
    println!("📝 Generating private key...");
    let key_output = Command::new("openssl")
        .args(["genpkey", "-algorithm", "RSA", "-out", &key_path])
        .output()?;
    ...
    let cert_output = Command::new("openssl")
        .args([
            ...
            "365",
            "-subj",
            "/CN=localhost/O=EnvoyControlPlane/C=US",
            "-addext",
            "subjectAltName=DNS:localhost,DNS:control-plane,IP:127.0.0.1",
        ])
        .output()?;
